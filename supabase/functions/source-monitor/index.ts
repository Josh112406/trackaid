...
const donationLanguage =
  /donate|donation|give now|support (our|the|this)|ways to give/i;
...
Deno.serve(async (request) => {
  ...
  const { data: secretIsValid } = await admin.rpc(
  ...
  const { data, error } = await admin
    ...
    .select(
      "id, organization_name, official_source_url, donation_url, source_domain, last_success_at, consecutive_failures",
    );
  if (error) return Response.json({ error: error.message }, { status: 500 });

  const results = await Promise.all(
    (data as Source[]).map(async (source) => {
      let statusCode: number | null = null;
      let donationCtaFound = false;
      let detail = "";
      try {
        const [sourceResponse, donationResponse] = await Promise.all([
          fetch(source.official_source_url, {
            redirect: "follow",
            signal: AbortSignal.timeout(12000),
          }),
          fetch(source.donation_url, {
            redirect: "follow",
            signal: AbortSignal.timeout(12000),
          }),
        ]);
        ...
        const destinationHost = new URL(
          donationResponse.url,
        ).hostname.toLowerCase();
        const recognizedDestination =
          destinationHost === source.source_domain ||
          destinationHost.endsWith(`.${source.source_domain}`);
        donationCtaFound =
          sourceResponse.ok &&
          donationResponse.ok &&
          recognizedDestination &&
          donationLanguage.test(sourceText);
        detail = donationCtaFound
          ? "Official page and recognized donation destination responded successfully."
          : "Donation call-to-action or recognized destination could not be confirmed.";
      } catch (monitorError) {
        detail =
          monitorError instanceof Error
            ? monitorError.message.slice(0, 900)
            : "Source check failed.";
      }

      const nextFailures = donationCtaFound
        ? 0
        : source.consecutive_failures + 1;
      const failedFor24Hours =
        !donationCtaFound &&
      ...
      await admin
        .from("external_campaign_sources")
        .update({
          source_health: donationCtaFound
            ? "healthy"
            : failedFor24Hours
              ? "unavailable"
              : "warning",
      ...
      return {
        ...
        healthy: donationCtaFound,
        hidden: failedFor24Hours,
      };
    }),
  );
  ...
  await admin.from("admin_audit_log").insert({
    ...
    detail: {
      ...
      hidden: results.filter((result) => result.hidden).length,
    },
  });
